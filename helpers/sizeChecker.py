...
parser.add_argument("--move", help="Put all wrong xml and images to a wrong_data folder inside each folder",
                    action="store_true")
args = parser.parse_args()

# if not isdir(train_directory) or not isdir(test_directory):
#     print(colored('[!]', 'yellow', attrs=['bold']),
#           colored('The training (%s) or test (%s) directories do not exist ' % (train_directory, test_directory)))
#     exit(1)
# else:
...
for tree in [directory]:
    if move and not isdir(tree + '/wrong_data'):
